...
SELECT name, continent, population FROM world WHERE continent IN(SELECT continent FROM world WHERE population IN(SELECT max(population) FROM world
GROUP BY continent) AND population <= 25000000)

-- 10. Some countries have populations more than three times that of any of their neighbours (in the same continent). Give the countries and continents.
SELECT x.name, x.continent FROM world x WHERE NOT EXISTS ( SELECT 1 FROM world y WHERE y.continent = x.continent AND y.name <> x.name AND y.population >= x.population/3 );